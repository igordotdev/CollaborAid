...
db.run(`
  CREATE TABLE IF NOT EXISTS naturalEntities (
  ...
  async fetch(req) {
    ...
    try {
      if (req.method === "GET" && url.pathname === "/api/naturalEntities") {
        ...
        const { query, params } = buildQuery("SELECT * FROM naturalEntities", filters);
        const users = db.query(query).all(...params);
        return new Response(JSON.stringify(users), { headers: corsHeaders });
      } else if (req.method === "POST" && url.pathname === "/api/naturalEntities") {
