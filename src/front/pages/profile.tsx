...
// Define the Company interface for typing
interface Company {
  NIP: string;
  REGON: string;
  name: string;
...
export const Profile: React.FC<ProfileProps> = ({ userId }) => {
  ...
  // Fetch company data from the backend using the userId prop
  const fetchCompanyData = async () => {
    try {
      if (!userId) {
        setError("User ID is required to fetch company data");
        return;
      }
      const response = await fetch(
        `http://localhost:3000/api/legalEntities?REGON=` + userId
      ); // Update with your actual endpoint
      if (!response.ok) {
        throw new Error("Network response was not ok");
      }
      const data = await response.json();
      setCompany(data[0]); // Store the data in stat
    } catch (error) {
  ...
  useEffect(() => {
    fetchCompanyData(); // Fetch company data when component mounts or when userId changes
